...
//					if (intrinsicWidth * values1[Matrix.MSCALE_X] < width
//							|| intrinsicWidth * values1[Matrix.MSCALE_X] <= (imgWidth - values1[Matrix.MTRANS_X])) {
...
//					}
//
//					if (intrinsicWidth * values1[Matrix.MSCALE_X] <= (imgWidth - values1[Matrix.MTRANS_X])) {
//
//						getParent().requestDisallowInterceptTouchEvent(false);
//
//					}
...
//        	Log.e("checkDxBound","imgWidth:"+imgWidth+"intrinsicWidth:"+intrinsicWidth+",intrinsicWidth * values[Matrix.MSCALE_X] :"+intrinsicWidth * values[Matrix.MSCALE_X]);
//        	Log.e("checkDxBound"," (imgWidth - values[Matrix.MTRANS_X]):"+ (imgWidth - values[Matrix.MTRANS_X]));

            float width = imgWidth;

            float matrixWidth = 0;
            if (values[Matrix.MTRANS_X] < 0) {
                matrixWidth = (imgWidth - values[Matrix.MTRANS_X]);
            } else {
                matrixWidth = (imgWidth + values[Matrix.MTRANS_X]);
            }
            if (intrinsicWidth * values[Matrix.MSCALE_X] <= matrixWidth) {
                getParent().requestDisallowInterceptTouchEvent(false);
...
//			if (intrinsicWidth * values[Matrix.MSCALE_X] <= (imgWidth - values[Matrix.MTRANS_X])) {
//
//				getParent().requestDisallowInterceptTouchEvent(false);
//
//			}
...
//            float width = imgWidth;  
    ...
    public void makeImageViewFit() {
        if (getScaleType() != ScaleType.MATRIX) {
            ...
            Log.e("makeImageViewFit", "makeImageViewFit");
            matrix.postScale(1.0f, 1.0f, imgWidth / 2, imgHeight / 2);
        }
    }


    public void makeImageViewFitPublic() {
        initUI();
        Log.e("makeImageViewFit", "makeImageViewFit");
        matrix.postScale(1.0f, 1.0f, imgWidth / 2, imgHeight / 2);
